...
class TemporalFocalLoss(nn.Module):
    """
    Combined loss function for imbalanced time series classification with temporal consistency
    """

    def __init__(self, alpha=0.25, gamma=2.0, temporal_weight=0.2):
        super().__init__()
        self.alpha = alpha
        self.gamma = gamma
    ...
    def forward(self, logits, targets):
        ...
        # Get probabilities for temporal consistency
        probs = F.softmax(logits, dim=1)
        minority_probs = probs[:, 1]  # Assuming class 1 is minority

        # Add penalty for sudden probability changes
        if len(minority_probs) > 1:
            temp_diff = torch.abs(minority_probs[1:] - minority_probs[:-1])
            temp_loss = torch.mean(temp_diff)
        else:
            temp_loss = torch.tensor(0.0, device=logits.device)

        # Combine losses with higher weight for minority class
        is_minority = (targets == 1).float()
        weighted_focal = torch.mean(focal_loss * (1 + is_minority))

        total_loss = weighted_focal + self.temporal_weight * temp_loss
        return total_loss


class AdaptiveThresholdLoss(nn.Module):
    def __init__(self, init_threshold=0.5, margin=0.1):
        super().__init__()
        # Initialize as Parameter to ensure it gets gradients
        self.threshold = nn.Parameter(torch.tensor([init_threshold], dtype=torch.float32)).to(device='cuda')
        self.margin = margin

    def forward(self, logits, targets):
        probs = torch.sigmoid(logits[:, 1]).to(device='cuda')

        # Explicitly calculate threshold loss
        threshold_pred = (probs > self.threshold).float()
        threshold_loss = F.binary_cross_entropy(threshold_pred, targets.float())

        # Standard BCE loss
        bce_loss = F.binary_cross_entropy_with_logits(logits[:, 1], targets.float())

        # Add gradient signal to update threshold
        probs_sorted = torch.sort(probs)[0]
        optimal_threshold_idx = int(len(probs_sorted) * (1 - targets.float().mean()))
        if optimal_threshold_idx < len(probs_sorted):
            optimal_threshold = probs_sorted[optimal_threshold_idx]
            threshold_update = F.mse_loss(self.threshold, optimal_threshold.detach())
        else:
            threshold_update = torch.tensor(0.0, device=logits.device)

        # Margin penalty
        threshold_penalty = torch.abs(self.threshold - 0.5) - self.margin
        threshold_penalty = F.relu(threshold_penalty)

        # Combine all terms
        total_loss = (bce_loss +
                      0.3 * threshold_loss +
                      0.2 * threshold_update +
                      0.1 * threshold_penalty)

        return total_loss, self.threshold.item()  # single .item() call converts tensor to scalar.item()


class CombinedTemporalLoss(nn.Module):
    def __init__(self, alpha=0.25, gamma=2.0, temporal_weight=0.2, init_threshold=0.5):
        super().__init__()
        self.focal_loss = TemporalFocalLoss(alpha, gamma, temporal_weight)
        self.threshold_loss = AdaptiveThresholdLoss(init_threshold)

    @property
    def threshold(self):
        return self.threshold_loss.threshold

    def forward(self, logits, targets):
        focal_loss = self.focal_loss(logits, targets)
        threshold_loss, threshold = self.threshold_loss(logits, targets)

        total_loss = focal_loss + threshold_loss
        return total_loss, threshold