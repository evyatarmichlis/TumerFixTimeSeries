...
# --- Configuration ---
INPUT_CSV = 'results/ekg_experiments/summary.csv'
OUTPUT_DIR = 'plots'
...
def plot_within_participant(df: pd.DataFrame, output_dir: str):
    ...
    Compares F1 scores across participants for different levels and strategies.
    """
    ...
    # Grab handles/labels from the auto-generated legend using the correct attribute name
...
def plot_cross_participant(df: pd.DataFrame, output_dir: str):
    ...
    One heatmap per strategy and metric level.
    """
    print("Plotting 'cross_participant' scope...")
    sub_df = df[df['scope'] == 'cross_participant'].copy()

    sub_df[['train_p', 'test_p']] = sub_df['experiment'].str.extract(r'train_(P\d+).*test_(P\d+)')

    if sub_df['train_p'].isnull().all():
        print("  - Could not parse train/test participants. Skipping.")
        return

    participants = sorted(df['experiment'][df['scope'] == 'within_participant'].unique())
